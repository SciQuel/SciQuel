...
export default function ContactArchiveDashboard() {
  ...
  async function unarchive(message: ContactMessage) {
    try {
      const res = await axios.patch(
        `${env.NEXT_PUBLIC_SITE_URL}/api/contact/${
          message.contactType === "FEEDBACK" ? "feedback" : "get_involved"
        }/${message.id}`,
        {
          new_status: "UNOPENED",
          send_reply: false,
      ...
      if (res.status === 200) {
        ...
        alert("Message has been un-archived!");
      }
    } catch (err) {
      console.error(err);
    }
        ...
        {messages.map((message) =>
          message ? (
            ...
            >
              ...
              <button
                onClick={() => {
                  unarchive(message).catch((err) => console.error(err));
                }}
                type="button"
